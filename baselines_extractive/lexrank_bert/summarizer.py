...
from lexrank_bert.power_method import stationary_distribution

sent_bert = SentenceTransformer('bert-base-nli-mean-tokens')
...
class LexRankBERT:
    ...
    def _calculate_similarity_matrix(self, sentences):
        sent_embs = sent_bert.encode(sentences)
        ...
        for i in range(length):
            for j in range(i, length):
                similarity = 1 - distance.cosine(sent_embs[i], sent_embs[j])
